...
**跨日進度清單**:
工作日誌裡跨日追蹤的工作項目，git 產不出來，所以存成一份檔案
（預設 `~/.claude/worklog-backlog.md`，不在任何 repo 裡）。寫日誌時只把未完成項目放進
「三、待辦事項」，定稿後把今天完成的項目更新為 ✅。
_Avoid_: backlog（這條線上「backlog」指的是 Slack 那張總表）、待辦列（那是 Slack 上的一列）
