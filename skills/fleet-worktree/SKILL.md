---
name: fleet-worktree
description: 接單 —— 把一個已發布的任務（issue）開成隔離的 worktree，派一個長駐 agent 進去做完。當使用者說「派工 #1234」「接這張單」「接這個任務」「開始做這個」「開 worktree 給這個」時使用；一張 issue 掛上 ready-for-agent 之後要動工時也用。
---

# 接單：一個任務，一個 worktree，一個負責人

一個已發布的**任務**（掛了 `ready-for-agent` 的 issue）＝ 一次接單。
...
當天做得完的任務不開 issue，你拿到的是一份 `drafts/…/….md`。差別只有兩點：

- **branch 名沒有編號** —— 用 `fix/<短名>`。掉的是「重複做同一件事會當場現形」那個保險，
  所以開工前 `git branch -a | grep <短名>` 看一眼。
- **做完 `slack-list ready` 回報 PM，然後把草稿刪掉。** 沒有 issue 要關。
...
issue 的「方向」那格是空的 —— 兩種做法都合理，只有使用者挑得出來。
**先跟他談完，談出結論才寫計畫**（兩件事併成同一關，不要談完再另外等一次核可）。

- **對齊期間不准改檔案。** 你坐在 worktree 裡，手會癢。
- **結論 `gh issue comment` 回去**，連同「考慮過 A/B、為什麼排除 A」那段推導。
- **談完的你就是負責人，一路做到底。** 這個任務不交接給別的 agent ——
  comment 帶得走「選了 A」，帶不走推導，而推導正是實作撞牆時要用的。

標 `⚠️ 範圍未定` 的不會走到這裡：那種在發布前就談完了，到你手上時已經是 `✅`。
...
**計畫寫進 issue、不是停在 pane 裡等人按** —— 這樣 N 份計畫可以一次看完，
跟一次看 N 個任務是同一個動作。互動式核可會讓 N 個 agent 各自隨機打斷使用者，
那正是這整套要避開的東西。
...
| 第二道 | 整合分支 | **驗收**：任務上寫的「怎樣算解完」達成了沒。有 e2e 覆蓋就跑 e2e，UI/UX 的主觀判斷才需要人看 |
